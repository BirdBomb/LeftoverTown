...
public class BuildingObj_TreeWithFruiter : BuildingObj
{
    ...
    [Header("Sprite结果树")]
    public Sprite[] sprites_Fruiter;
    [SerializeField, Header("生长周期(小时)")]
    private int int_GrowthTime = 1;
    [SerializeField, Header("结果周期(小时)")]
    private int int_FruitTime = 1;
    [SerializeField, Header("成年树生命值")]
    private int int_TreeHp = 0;
    [SerializeField, Header("树桩生命值")]
    private int int_StumpHp = 0;
    ...
    [SerializeField, Header("果树基本掉落物")]
    private List<BaseLootInfo> baseLootInfos_Fruiter = new List<BaseLootInfo>();
    [SerializeField, Header("果树额外掉落物")]
    private List<ExtraLootInfo> extraLootInfos_Fruiter = new List<ExtraLootInfo>();
    ...
    public void All_CompareTime()
    {
        ...
        else if (treeState == TreeState.Tree)
        {
            ...
            else if (gameTime_Now - gameTime_Sign > int_GrowthTime + int_FruitTime)
            {
                All_UpdateTreeState(TreeState.Fruiter);
            }
        }
        else if (treeState == TreeState.Fruiter)
        {
            if (gameTime_Now - gameTime_Sign <= int_GrowthTime)
            {
                All_UpdateTreeState(TreeState.Stump);
    ...
    public override void All_Broken()
    {
        ...
        else if (treeState == TreeState.Fruiter)
        {
            ...
            {
                Local_ChangeInfo((gameTime_Now).ToString());
                State_CreateLootItem(State_GetLootItem(baseLootInfos_Fruiter, extraLootInfos_Fruiter));
            }
        }
