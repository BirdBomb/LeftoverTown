...
public class ItemLocalObj_ScatterGun : ItemLocalObj
{
    [SerializeField]
    ...
    [SerializeField]
    private Transform transform_ScatterGun;
    ...
    [SerializeField, Header("射击散射数量")]
    private int config_BulletCount;
    ...
    public void Shoot(short bulletID, List<Vector3> dirList)
    {
        for (int i = 0; i < dirList.Count; i++)
        {
            ...
            bulletBase.InitBullet(dirList[i], 0, 5, actorManager.actorNetManager);
        }
        ...
        transform_ScatterGun.DOKill();
        transform_ScatterGun.localPosition = Vector3.zero;
        transform_ScatterGun.DOPunchPosition(vector3_KickBack, config_ShotCD).OnComplete(() =>
        {
        ...
        transform_ScatterGun.localRotation = Quaternion.identity;
        transform_ScatterGun.DOPunchRotation(new Vector3(0, 0, float_KickOn), config_ShotCD);
    }
    public void Dull()
    {
